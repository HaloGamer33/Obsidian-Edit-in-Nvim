    ...
    constructor(
        ...
        if (settings.pathToBinary) {
            ...
            if (found.matches.length > 0) {
                ...
                this.nvimBinary = {
                    path: '',
                    error: new Error('Neovim binary not found automatically.'),
                };
                new Notice(
                    'Edit In Neovim: Could not automatically find a Neovim binary. Please set the path manually in settings or ensure nvim is in your PATH.',
                    10000,
                );
            }
        }
  ...
  - Error State: ${this.nvimBinary.error ? this.nvimBinary.error.message : 'None'}`);
    ...
    async newInstance(adapter: FileSystemAdapter) {
        ...
        if (process.platform === 'win32') {
            const terminalName =
                termPath.split('\\').pop()?.toLowerCase() || '';

            if (
                terminalName === 'alacritty.exe' ||
                terminalName === 'wezterm.exe' ||
                terminalName === 'kitty.exe'
            ) {
                ...
                terminalName === 'powershell.exe' ||
                terminalName === 'pwsh.exe'
            ) {
        ...
        try {
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    new Notice(
                        `Failed to establish RPC connection: ${rpcError.message}`,
                        7000,
                    );
                    this.close();
                }
            ...
            new Notice(
                `Error trying to spawn Neovim: ${spawnError.message}`,
                10000,
            );
            this.process = undefined;
            this.instance = undefined;
    ...
    openFile = async (file: TFile | null) => {
        ...
        let isSupported = this.settings.supportedFileTypes.includes(
            file.extension,
        );
        ...
        if (this.instance && this.process) {
            ...
            try {
                if (await isPortInUse(port)) {
                    ...
                    new Notice(
                        `Opening file in external Neovim on ${listenAddress}...`,
                        3000,
                    );
                    canConnect = true;
                } else {
        ...
        if (!canConnect && this.settings.warnNoNvimOnNote) {
            ...
            new Notice(
                "No running Neovim found. Use 'Open Neovim' or ensure an external instance is listening.",
                5000,
            );
            return;
        }
        ...
        try {
            ...
            new Notice(
                `Failed to run Neovim command: ${execFileError.message}`,
                10000,
            );
        }
    };
